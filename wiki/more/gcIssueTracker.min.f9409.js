var spinner_html = "<img class='spinner' src='https://www.google.com/ig/images/spinner.gif' width='16' height='16' />";
...
function ellipsifyWords(C, A) {
    var B;
    words_arr = C.split(" ");
    for (B = 0; B < words_arr.length; B++) {
        words_arr[B] = ellipsify(words_arr[B], A)
    }
    return words_arr.join(" ")
}
var prefs_obj = new gadgets.Prefs();
var searchURL_tmpl = "https://code.google.com/p/%PROJECT%/issues/list?q=%SEARCH%";
var title_tmpl = "%PROJECT% issues";
var urlMaster_tmpl = "https://code.google.com/p/%PROJECT%/issues/list?sort=%SORT%&colspec=%COLSPEC%&num=%NUM%&start=%START%";
...
function render(A) {
    ...
    if (!singleProject_bool) {
        for (var C = 0; C < totalProjects_num; C++) {
            var B = url_tmpl.replace("%PROJECT%", otherProjects_arr[C]);
            fetchAsStringLegacy(B, responseHandler)
        }
    }
    responseHandler(A, true)
}
function responseHandler(A, D) {
    if (A.search(invalid_re) != -1) {
        var C = new gadgets.MiniMessage(moduleID_num);
        C.createTimerMessage(invalidProject_tmpl.replace("%PROJECT%", scrapeInvalidProjectName(A)), 5);
        totalProjects_num--;
        return
    }
    if (D) {
        var B = jQuery(".pagination:first", jQuery(A)).html();
        totalItems_str = B == null ? "0" : new String(B.match(/.*of.*/)).replace(/.*of\ /, "")
    }
    if (A.search(/Your search did not generate any results./) == -1) {
        var E = jQuery("#resultstable", jQuery(A));
        scrapeContent(E, scrapeProjectName(A))
    }
    if (singleProject_bool) {
        inject(A)
    } else {
        if (++retrievedProjects_num == (totalProjects_num + 1)) {
            inject("")
        }
    }
}
function scrapeProjectName(B) {
    var A = /<title>\s*[^\s]+\s*-\s*([^\s]+)/im;
    var C = B.match(A);
    if (C.length == 2) {
        return C[1]
    } else {
...
function scrapeInvalidProjectName(A) {
    var B = A.search("/p/");
    var C = A.search("/issues/");
    if (B == -1 || C == -1) {
        return""
    }
    B += 3;
...
function inject(B) {
    var D = "<form><select id='selectType'><option value='all'>All Issues</option>";
    if (_args()["synd"] == "ig") {
        D += "<option value='assignedToMe'>Issues Assigned to Me</option>";
        D += "<option value='reportedByMe'>Issues Reported by Me</option>"
    }
    D += "<option value='starred'>Important Issues</option></select></form>";
    var C = "";
    C += D;
    C += buildContent();
    if (singleProject_bool) {
        var A = generatePagingFromPage(B, start_num);
        C += "<br />" + A
    } else {
        var F = Math.ceil(issueData_arr.length / 10);
        F = F > 4 ? 4 : F;
        var E = issueData_arr.length > 40;
        if (F > 0) {
            C += "<br />" + generatePaging(F, currentPage_num, E, totalItems_str)
        }
    }
    $("#content_div").html(C);
    if (singleProject_bool) {
...
function scrapeContent(E, A) {
    var B = "";
    var C = "";
    var D = "";
    E.find("tr").each(function (F) {
        jQuery(this).find("td:not(:last):not(:first)").each(function (G) {
            switch (G) {
            case 0:
                B = jQuery(this).text();
                break;
        ...
        if (B != "" && D != "") {
            issueData_arr[issueData_arr.length++] = new issueData(C, B, D, A)
        }
    })
}
function buildContent() {
    var B = "<td valign='top'><a href='https://code.google.com/p/%PROJECT%/issues/detail?id=%ID%' target='_blank'>%ID%</a>%CONTENT%</td>";
    var G = "<td valign='top' style='white-space: nowrap; border-top: 1px solid #BBBBBB;'>%CONTENT%</td>";
    var C = "<td valign='top'>%CONTENT%</td>";
    var I = "<td valign='top' style='border-top: 1px solid #BBBBBB;'><a href='https://code.google.com/p/%PROJECT%/issues/detail?id=%ID%' target='_blank'>%ID%</a>%CONTENT%</td>";
    var A = "<td valign='top'style='border-top: 1px solid #BBBBBB;'>%CONTENT%</td>";
    var D = "";
    var F;
    ...
    D += "<center><table cellspacing='0' cellpadding='0' width='100%' id='#resultstable' style='border-bottom: 1px solid #BBBBBB;'>";
    var E = 0;
    var H = issueData_arr.length;
    if (!singleProject_bool) {
        E = (currentPage_num * 10) - 10;
        H = currentPage_num * 10;
        H = H <= issueData_arr.length ? H : issueData_arr.length
    }
    for (F = E; F < H; F++) {
        D += "<tr>";
        if (!singleProject_bool) {
            D += G.replace("%CONTENT%", issueData_arr[F].sort_str.replace(/\ *ago/, ""));
            D += G.replace("%CONTENT%", issueData_arr[F].projectName_str);
            D += "</tr><tr>";
            D += B.replace("%PROJECT%", issueData_arr[F].projectName_str).replace("%ID%", issueData_arr[F].id_str).replace("%ID%", issueData_arr[F].id_str).replace("%CONTENT%", "");
            D += C.replace("%CONTENT%", ellipsifyWords(issueData_arr[F].summary_str.replace(/[\n\r\ ]*$/, ""), 20))
        } else {
            D += I.replace("%PROJECT%", issueData_arr[F].projectName_str).replace("%ID%", issueData_arr[F].id_str).replace("%ID%", issueData_arr[F].id_str).replace("%CONTENT%", "");
            D += G.replace("%CONTENT%", issueData_arr[F].sort_str.replace(/\ *ago/, ""));
            D += A.replace("%CONTENT%", ellipsifyWords(issueData_arr[F].summary_str.replace(/[\n\r\ ]*$/, ""), 20))
        }
        D += "</tr>"
    }
    D += "</table></center>";
...
function pagingCallbackSingle(A) {
    if (A == PAGE_MORE) {
        moreURL_str = urlMaster_tmpl.replace("%SORT%", sortBy_str).replace("&colspec=%COLSPEC%", "").replace("%START%", 40).replace("&num=%NUM%", "").replace("%PROJECT%", projectName_str);
...
function pagingCallbackMultiple(A) {
    if (A == PAGE_MORE) {
        moreURL_str = urlMaster_tmpl.replace("%SORT%", sortBy_str).replace("&colspec=%COLSPEC%", "").replace("%START%", 0).replace("&num=%NUM%", "").replace("%PROJECT%", projectName_str);
        window.open(moreURL_str, "_blank");
        return
    }
    if (A == PAGE_PREV) {
...
function addSelectEvent() {
    ...
    $("#selectType").change(function (B) {
        var C = jQuery(this).val();
        if (userName_str == "" && (C == "assignedToMe" || C == "reportedByMe")) {
            var A = new gadgets.MiniMessage(moduleID_num);
            A.createTimerMessage("To use this feature, please enter your user name in the preferences of this gadget.", 4);
            jQuery("#selectType").val(issueType_str)
        } else {
            issueType_str = C;
            prefs_obj.set("issueType", C);
            setFilteredSearch();
            localInit()
        }
    })
...
function issueData(C, A, D, B) {
    this.sort_str = C;
    this.id_str = A;
    this.summary_str = D;
    this.projectName_str = B
}
function sortIssueData(B, A) {
    var D = B.sort_str.toLowerCase();
    var C = A.sort_str.toLowerCase();
    return((D < C) ? -1 : ((D > C) ? 1 : 0))
}
function sortIssueDataByDate(B, A) {
    var H = B.sort_str.toLowerCase();
    var D = A.sort_str.toLowerCase();
    var E = rateDate(H);
    var F = rateDate(D);
    if (E == F) {
        if (E == 0) {
...
function rateDate(A) {
    if (A.search(/moments/) != -1) {
        return 0
    }
    if (A.search(/minutes/) != -1) {
        return 1
    }
    if (A.search(/hours/) != -1) {
        return 2
    ...
    if (A.search(/[a-z]{3}\ [0-9]{2}$/) != -1) {
        return 4
    }
    if (A.search(/[a-z]{3}\ [0-9]{4}]/) != -1) {
        return 5
    }
    return 6
}
