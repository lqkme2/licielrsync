...
function fetch(A, E, C, B) {
    ...
    //if (B) {
    D[gadgets.io.RequestParameters.NUM_ENTRIES] = 10
    //}
...
var url_tmpl = "https://code.google.com/p/licielrsync/source/list";
var title_tmpl = "%PROJECT% changes";
var feed_tmpl = "https://code.google.com/feeds/p/licielrsync/svnchanges/basic?path=/trunk";
...
var feedURL_str = "";
var path_str = prefs_obj.getString("path");
function render(A) {
    feedURL_str = feed_tmpl.replace("%PROJECT%", _hesc(projectName_str)).replace("%PATH%", _hesc(path_str));
    fetchAsFeed(feedURL_str, renderCallback)
}
function renderCallback(J) {
    var I = "";
    if (J.data) {
        I += "<center><table cellspacing='0' cellpadding='0' width='100%' id='#resultstable'>";
        var H = "";
        var B = "";
        var F;
        var A = Math.ceil(J.data.Entry.length / 5);
        A = Math.min(A, 3);
        var G = J.data.Entry.length > 15;
        var D = (currentPage_num - 1) * 5;
        var C = Math.min(D + 10, J.data.Entry.length);
        for (var E = D; E < C; E++) {
            H = J.data.Entry[E].Title;
            F = H.search(":");
            B = "r" + H.substring(9, F);
            H = H.substring(F + 2, H.length);
            H = ellipsify(H, 100);
            I += row_tmpl.replace("%REVISION%", _hesc(B)).replace("%TITLE%", _hesc(H)).replace("%LINK%", _hesc(J.data.Entry[E].Link)).replace("http://", "https://");
        }
        I += "</table>";
        //I += generatePaging(A, currentPage_num, G, J.data.Entry.length);
        I += "</center>"
    } else {
        I += "No changes could be found."
    }
    $("#content_div").html(I);
    addPagingEvents(pagingCallback);
    adjustIFrameHeight()
}
function pagingCallback(A) {
    if (A == PAGE_MORE) {
    ...
    fetchAsJson(feedURL_str, renderCallback, 16, true)
}
